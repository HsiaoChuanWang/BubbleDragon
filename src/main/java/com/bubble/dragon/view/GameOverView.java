...
public final class GameOverView {
    ...
    public Scene createScene() {
        Label icon = new Label(victory ? "★" : "×");
        icon.getStyleClass().add(victory ? "victory-icon" : "defeat-icon");
        Label title = new Label(victory ? "闖關成功！" : "遊戲結束");
        title.getStyleClass().add("result-title");
        Label message = new Label(victory ? "所有敵人都被泡泡打敗了！" : "別灰心，小龍準備好再試一次。 ");
        message.getStyleClass().add("subtitle");
        Button retry = new Button("再玩一次");
        retry.getStyleClass().add("primary-button");
        retry.setOnAction(e -> app.startGame());
        Button home = new Button("返回首頁");
        home.getStyleClass().add("secondary-button");
        home.setOnAction(e -> app.showHome());
        HBox buttons = new HBox(16, retry, home);
        buttons.setAlignment(Pos.CENTER);
        VBox root = new VBox(22, icon, title, message, buttons);
        root.setAlignment(Pos.CENTER);
        root.getStyleClass().addAll("screen", "result-screen");
